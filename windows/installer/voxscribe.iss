; VoxScribe — Windows installer.
;
; Build:  dotnet publish src\VoxScribe.App -c Release -r win-x64 --self-contained true `
;                        -p:PublishSingleFile=true -o publish
;         iscc installer\voxscribe.iss
;
; Per-user on purpose: no UAC prompt, and the app already keeps everything per-user
; (settings, transcripts and the speech model live under %LOCALAPPDATA%\VoxScribe). With
...
AppPublisher=VoxScribe
DefaultDirName={autopf}\{#MyAppName}
DisableProgramGroupPage=yes
PrivilegesRequired=lowest
OutputDir=Output
OutputBaseFilename=VoxScribe-Setup-{#MyAppVersion}
SetupIconFile=..\src\VoxScribe.App\Assets\app.ico
...
[InstallDelete]
; The executable was called Murmur.App.exe until the rename. The AppId is unchanged, so this
; is an upgrade in place and Inno leaves unknown files behind — including a second, stale copy
; of the app that a Start-menu shortcut or a login entry could still be pointing at.
Type: files; Name: "{app}\Murmur.App.exe"
