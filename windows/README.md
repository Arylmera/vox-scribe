...
├─ global.json                    SDK pin
├─ src/
│  ├─ VoxScribe.Dictionary/          corrections + biasing          net10.0
│  ├─ VoxScribe.Abstractions/        the four platform interfaces   net10.0
│  ├─ VoxScribe.Core/                engine, segmenter, storage     net10.0
│  ├─ VoxScribe.Speech/              Parakeet via sherpa-onnx       net10.0
│  ├─ VoxScribe.Testing/             fakes for the interfaces       net10.0
...
`VoxScribe.App` loads the platform layer **by reflection** rather than referencing it. A direct
...
```bash
cd windows
dotnet build VoxScribe.sln --no-incremental -warnaserror
dotnet test  VoxScribe.sln
```

**On macOS or Linux** — use the solution filter. `VoxScribe.Platform.Windows` targets
